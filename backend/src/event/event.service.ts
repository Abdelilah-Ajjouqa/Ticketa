...
export class EventService {
    ...
    async findAll(isAdmin: boolean = false) {
        ...
        return this.eventModel.find(filter).populate('createdBy', 'username email').sort({ date: 1 });
    }

    async findOne(id: string, isAdmin: boolean = false) {
        const event = await this.eventModel.findById(id).populate('createdBy', 'username email');
